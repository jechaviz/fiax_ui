...
export default {
  ...
  methods: {
    ...
    applyNamedView(viewId, markClean = true) {
      ...
      this.sorting = Array.isArray(view.config.sorting) && view.config.sorting.length ? view.config.sorting.map((entry) => ({ ...entry })) : [{ id: 'date', dir: 'desc' }];
    ...
    async saveEdit(row, column, nextValue) {
      ...
      const api = window.fiax?.api;
      if (!api?.saveInline || !this.surfaceKey) {
        this.$emit('save-inline', {
