<template>
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 h-full flex flex-col">
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
      <div>
        <h1 class="text-3xl font-bold tracking-tight text-white">Nómina CFDI</h1>
        <p class="text-slate-400 mt-1">Gestión de recibos de nómina para empleados.</p>
      </div>
    </header>
...
export default {
  ...
  setup(props) {
    ...
    function createNew() {
      ...
      if (router) router.push('/cfdi/nomina/new');
    }

    function handleRowAction(payload) {
      if(payload.action === 'open-record' || payload.action === 'row-click') {
        const router = window.fiax?._router;
        if (router) router.push('/cfdi/nomina/' + payload.row.id);
      }
    }

    return { filteredPayroll, createNew, handleRowAction };
  }
}
</script>
